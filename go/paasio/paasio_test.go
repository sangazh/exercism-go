...
func testRead(t *testing.T, reader func(io.Reader) ReadCounter) {
	...
	n, nops := rc.ReadCount()
	if n != int64(chunkLen) {
		t.Fatalf("reported %d bytes readCounter instead of %d", n, chunkLen)
	}
	if nops < 2 {
...
func testReadTotal(t *testing.T, rc ReadCounter) {
	...
	n, nops := rc.ReadCount()
	if n != totalBytes {
		t.Errorf("expected %d bytes readCounter; %d bytes reported", totalBytes, n)
	}
	if nops != numGo {
		t.Errorf("expected %d readCounter operations; %d operations reported", numGo, nops)
	}
}
	...
	for i := 0; i < numGo; i++ {
		...
		go func() {
			...
			if int64(nops) != expectedOps {
				t.Errorf("expected %d ops@%d bytes readCounter; %d ops reported", expectedOps, n, nops)
			}
			wg.Done()
		}()
	}
